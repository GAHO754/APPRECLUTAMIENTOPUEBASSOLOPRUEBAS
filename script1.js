// =============================================
// script1.js — flujo original + barra deslizadora (scroll contenedor)
// =============================================
...
const { jsPDF } = window.jspdf; // Mantener al inicio
...
window.onload = () => {
  ...
  // Contenedor título + botón Regresar
...
function processImageWithOpenCV(canvasElement, docName) {
  ...
  try {
    ...
    if (bestContour) {
      ...
      let [tl, tr, br, bl] = orderPoints(points);
      ...
      srcCoords.delete(); destCoords.delete(); M.delete(); bestContour.delete();
      ...
      if (docName === "Contrato laboral") {
        ...
        tempGrayForBinarization.delete(); binarizedMat.delete();
      } else {
...
// =============================================
// Barra deslizadora: ahora controla el SCROLL del contenedor
// =============================================
// Mantengo el mismo nombre para no tocar tu llamada.
function attachScrollStrip(/* cropperIgnorado */) {
  const strip = document.getElementById('cropper-scroll-strip');
  const thumb = document.getElementById('scroll-thumb');
  const container = document.getElementById('cropper-container');
  if (!strip || !thumb || !container) return;
  ...
  function updateThumb() {
    ...
    const ratio = ch / sh;                 // proporción visible
    const minThumb = 36;
    const h = Math.max(minThumb, Math.round(ratio * (strip.clientHeight - 4)));
    thumb.style.height = h + 'px';

    const maxScroll = sh - ch;
    const usable = strip.clientHeight - h - 4; // pista útil (márgenes 2px arriba/abajo)
    const y = (container.scrollTop / maxScroll) * usable;
    thumb.style.transform = `translateY(${Math.max(2, 2 + y)}px)`;
  }

  function scrollToThumbEvent(e) {
    const rect = strip.getBoundingClientRect();
    let y = e.clientY - rect.top;
    y = Math.max(0, Math.min(rect.height, y));
    const h = thumb.offsetHeight;
    const usable = rect.height - h - 4;
    const t = (y - h / 2 - 2) / usable;       // 0..1
    const clamped = Math.max(0, Math.min(1, t));
    const maxScroll = container.scrollHeight - container.clientHeight;
    container.scrollTop = clamped * maxScroll;
  }

  // Arrastre del pulgar / clic en la pista
  let dragging = false;
  const onPointerDown = (e) => { dragging = true; strip.setPointerCapture(e.pointerId); scrollToThumbEvent(e); };
  const onPointerMove = (e) => { if (!dragging) return; scrollToThumbEvent(e); };
  ...
  // Rueda del mouse sobre la barra => scroll del contenedor
  strip.addEventListener('wheel', (e) => {
    e.preventDefault();
    container.scrollBy({ top: e.deltaY });
  }, { passive: false });

  // Al hacer scroll en el contenedor, actualiza el pulgar
  container.addEventListener('scroll', updateThumb);

  // Recalcular si cambia el tamaño del contenedor o su contenido
...
function openCrop(docName) {
  ...
  // (Opcional) primer clic en la pista hace scroll suave
  if (strip && container) {
  ...
  cropperImg.onload = () => {
    if (cropperImg.src) {
      cropper = new Cropper(cropperImg, {
        // Imagen fija: sin zoom ni paneo
        viewMode: 1,
        autoCropArea: 0.8,
        background: false,
        responsive: true,

        movable: false,          // ❌ no mover imagen
        zoomable: false,         // ❌ no zoom
        zoomOnWheel: false,      // ❌ no zoom con rueda
        zoomOnTouch: false,      // ❌ no zoom con gesto
        scalable: false,         // ❌ no escalar

        dragMode: 'crop',        // ✅ mover/redimensionar recuadro de recorte
        toggleDragModeOnDblclick: false
        // cropBoxMovable/resizable quedan true por defecto
      });

      // Conectar barra que ahora solo scrollea el contenedor
      attachScrollStrip(cropper); // el parámetro se ignora por compatibilidad
    }
  };
  cropperImg.onerror = () => {
...
function confirmCrop() {
  if (!cropper) { alert("Cropper no está activo."); return; }

  const canvas = cropper.getCroppedCanvas();
  if (!canvas) { alert("No se pudo obtener el área recortada."); return; }
  ...
  for (const [docName, imageData] of Object.entries(scannedImages)) {
    const promise = new Promise((resolve) => {
      ...
      img.onload = function () {
        ...
        pdfOutput = pdf.output('blob'); // Mantengo tu variable como la usabas
